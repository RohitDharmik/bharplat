...
import { CreditCard, Wallet, Banknote, Landmark, Check, ArrowLeft, Tag, Pencil, Lock, ChevronRight } from 'lucide-react';
...
export const PaymentsView: React.FC<PaymentsViewProps> = ({ orders, tables, onProcessPayment }) => {
  ...
  const getItemImage = (menuItemId: string) => {
    return menu.find(m => m.id === menuItemId)?.image || 'https://picsum.photos/200/200';
  };
      ...
      <div className="animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="flex justify-between items-start mb-8">
            <div>
                ...
                <h2 className="text-4xl font-bold text-white mb-2">{selectedTable.name}</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                            {selectedOrder.items.map((item, idx) => (
                                <div key={idx} className="grid grid-cols-12 items-center py-4 border-b border-white/5 last:border-0 hover:bg-white/5 rounded-lg px-2 transition-colors group">
                                    <div className="col-span-6 flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-neutral-500">{item.notes || getItemDescription(item.menuItemId)}</p>
                                        </div>
                                    </div>
                                    <div className="col-span-2 text-center text-white font-medium">{item.quantity}</div>
                                    <div className="col-span-2 text-right text-neutral-400 text-sm">₹{item.price.toFixed(2)}</div>
                                    <div className="col-span-2 text-right text-white font-bold">₹{(item.price * item.quantity).toFixed(2)}</div>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className="lg:col-span-5 space-y-6">
                 ...
                 <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button className="p-4 bg-[#1E1E1E] border-2 border-gold-500 rounded-xl flex flex-col items-center justify-center gap-2 text-gold-500 shadow-lg shadow-gold-500/10 relative">
                             <div className="absolute top-2 right-2 text-gold-500"><CheckCircleIcon /></div>
                             <CreditCard size={24} />
                 ...
                 <div className="space-y-4">
                     <div>
                        <label className="text-xs text-neutral-500 font-bold uppercase mb-1.5 block tracking-wider">Card Number</label>
                        <div className="relative">
                            <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" size={18} />
                            <input type="text" placeholder="0000 0000 0000 0000" className="w-full bg-[#121212] border border-white/10 rounded-lg py-3 pl-10 pr-4 text-white placeholder-neutral-700 focus:border-gold-500 focus:outline-none transition-colors" />
                        </div>
                     </div>
                     <div className="grid grid-cols-2 gap-4">
                         <div>
                            <label className="text-xs text-neutral-500 font-bold uppercase mb-1.5 block tracking-wider">Expiry</label>
                            <input type="text" placeholder="MM/YY" className="w-full bg-[#121212] border border-white/10 rounded-lg py-3 px-4 text-white placeholder-neutral-700 focus:border-gold-500 focus:outline-none transition-colors" />
                         </div>
                         <div>
                            <label className="text-xs text-neutral-500 font-bold uppercase mb-1.5 block tracking-wider">CVC</label>
                            <div className="relative">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" size={16} />
                                <input type="text" placeholder="123" className="w-full bg-[#121212] border border-white/10 rounded-lg py-3 pl-10 pr-4 text-white placeholder-neutral-700 focus:border-gold-500 focus:outline-none transition-colors" />
                            </div>
                         </div>
                     </div>
                 </div>
                 ...
                 {/* Discount */}
                 <div>
                    <label className="text-xs text-neutral-500 font-bold uppercase mb-1.5 block tracking-wider">Discount Code</label>
                    <div className="flex gap-2">
                         <input type="text" placeholder="Enter coupon code" className="flex-1 bg-[#121212] border border-white/10 rounded-lg py-3 px-4 text-white placeholder-neutral-700 focus:border-gold-500 focus:outline-none transition-colors" />
                         <button className="bg-white/10 hover:bg-white/20 text-white px-6 rounded-lg font-medium transition-colors">Apply</button>
                    </div>
                 </div>
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-end pt-4">
                        <div>
                             <span className="text-xs text-neutral-500 font-bold uppercase tracking-wider block mb-1">Total Amount Due</span>
                             <span className="text-xs text-neutral-600">Includes all taxes</span>
                        </div>
                        <span className="text-4xl font-black text-gold-500">₹{totalDue.toFixed(2)}</span>
                    </div>
                 </div>
          ...
          return (
            ...
            >
              <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center w-full">
                <div>
                  <h3 className="font-bold text-white text-lg group-hover:text-gold-500 transition-colors">{table?.name || 'Unknown Table'}</h3>
                  <span className="text-xs text-neutral-400">Order #{order.id}</span>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-gold-400">₹{order.totalAmount}</p>
                </div>
              </div>
              ...
              <div className="p-4 flex-1 w-full">
                <ul className="space-y-2 mb-4">
                  ...
                  {order.items.length > 3 && (
                    <li className="text-xs text-neutral-500 italic">+ {order.items.length - 3} more items</li>
                  )}
                </ul>
                <div className="border-t border-white/5 pt-2 flex justify-between text-xs text-neutral-500">
                   <span className="flex items-center gap-1"><Check size={12} /> Ready for Payment</span>
...
const CheckCircleIcon = () => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M7.75 11.9999L10.58 14.8299L16.25 9.16992" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
);