...
export const UsersView: React.FC = () => {
  ...
  const attendanceColumns = [
      ...
      },
      {
          title: 'Employee',
          dataIndex: 'userId',
          render: (userId: string) => users.find(u => u.id === userId)?.name || userId
      },
      {
          title: 'Status',
          dataIndex: 'status',
          render: (status: string) => <Tag color={status === 'Present' ? 'green' : 'red'}>{status}</Tag>
      },
      {
          title: 'Check In',
          dataIndex: 'checkIn'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-neutral-900 dark:text-white">Staff Management</h2>
          <p className="text-neutral-500 dark:text-neutral-400 text-sm">Manage access, roles, and attendance</p>
        </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[600px]">
                      <thead>
                        <tr className="border-b border-neutral-200 dark:border-white/5 bg-neutral-50 dark:bg-white/5">
                          <th className="p-4 font-semibold text-neutral-600 dark:text-neutral-300 text-sm">User</th>
                          <th className="p-4 font-semibold text-neutral-600 dark:text-neutral-300 text-sm">Role</th>
                          <th className="p-4 font-semibold text-neutral-600 dark:text-neutral-300 text-sm">Status</th>
                          <th className="p-4 font-semibold text-neutral-600 dark:text-neutral-300 text-sm">Last Active</th>
                          <th className="p-4 font-semibold text-neutral-600 dark:text-neutral-300 text-sm text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {users.map((user) => (
                          <tr key={user.id} className="border-b border-neutral-200 dark:border-white/5 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors group">
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-neutral-200 to-neutral-300 dark:from-neutral-700 dark:to-neutral-800 flex items-center justify-center border border-white/10">
                                  <span className="font-bold text-neutral-600 dark:text-gold-400">{user.name.split(' ').map(n => n[0]).join('')}</span>
                                </div>
                                <div>
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-2">
                                <Shield size={14} className="text-gold-600 dark:text-gold-500" />
                                <span className="text-sm text-neutral-600 dark:text-neutral-300">{user.role}</span>
                              </div>
                            </td>
                            <td className="p-4">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                ${user.status === 'Active' ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-red-500/10 text-red-600 dark:text-red-400'}
                              `}>
                                <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${user.status === 'Active' ? 'bg-green-500 dark:bg-green-400' : 'bg-red-500 dark:bg-red-400'}`}></span>
                                {user.status}
                              </span>
                            </td>
                            <td className="p-4">
                ...
                children: (
                    <div>
                        <div className="mb-4 flex justify-end">
                            <button className="text-xs bg-neutral-100 dark:bg-white/10 px-3 py-1 rounded">Download Report</button>
                        </div>
                        <Table 
